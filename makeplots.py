...
def run_grid(num, stip_mult, og_inc, keep_sample=False):
    ...
    ensemble = EnsemblePair(num, stip_mult, og_inc=og_inc, rng_seed=rng_seed,
                            vlim=vlim, save_simulation_pairs=True,
                            randomize_inclinations=True)
    ensemble.sample('grid', sma_min=1., sma_max=100.)
    ensemble.run()
    fig, _ = ensemble.heatmaps(mask=True, draw_boundary=False, output=True)
...
def main(argv):
    ...
    run_grid(num_per_ensemble, stip_multiplicity, giant_inclination)
